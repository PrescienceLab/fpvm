...
// does not support compressed format
// focuses specifically on FP instructions F, D, Q for now, particularly D

typedef struct riscv_alltype {
  uint8_t  opcode:7;
  uint32_t rest:25;
} riscv_alltype_t;

typedef struct riscv_rtype {
  uint8_t opcode:7;
  uint8_t rd:5;
  uint8_t funct3:3;
  uint8_t rs1:5;
...
// four register variant of r type
typedef struct riscv_r4type {
  uint8_t opcode:7;
  uint8_t rd:5;
  uint8_t funct3:3;
  uint8_t rs1:5;
...
// this is an s type, but the immediate
// is a branch target and thus a multiple of
// 2, so the low order immediate bit becomes
// the high-order bit...
typedef struct riscv_btype {
  uint8_t opcode:7;
  uint8_t imm11:1;
  uint8_t imm4_1:4;
  uint8_t funct3:3;
  ...
  uint8_t imm10_5:6;
  uint8_t imm12:1;
} __attribute__((packed)) riscv_btype_t;

// this is an immediate that is
// shifted left by 12 (conveniently that's
// its location in the instruction)
typedef struct riscv_utype {
  uint8_t opcode:7;
  uint8_t rd:5;
  uint32_t imm31_12:20;
} __attribute__((packed)) riscv_utype_t;

// this is a u type, but a j immediate
// is shifted left by 1
typedef struct riscv_jtype {
  uint8_t opcode:7;
  uint8_t rd:5;
  uint8_t imm19_12:8;
  uint8_t imm11:1;
...
// shared between RV64F and RV64D and RV64D
#define OP_FL      0x07
#define OP_FS      0x27
...
#define OP_FOPGEN  0x53

// floating point loads are  l types, with funct3 telling you the order
// floating point stores are s types, with funct3 telling you the order
#define F3_FLS_H    0x1  // half
#define F3_FLS_W    0x2  // float
#define F3_FLS_D    0x3  // double
#define F3_FLS_Q    0x4  // quad

// floating point madds/etc are r4 types with funct2 telling you size,
// but not the order...   
#define F2_W  0x0  // float
#define F2_D  0x1  // double
#define F2_H  0x2  // half
#define F2_Q  0x3  // quad

// This convention is used for the other
// sizes as well, with F7 split into F2 (size) and F5 (other)
#define F7_TO_F2(f) ((f)&0x3)
#define F7_TO_F5(f) ((f)>>2)

// floating point generals are r types with low order 2 bits of
// funct7 telling you the size (above) and high order 5 bits telling
// you the operation (here).    For a 2 operand instruction, rs2
// tells you the variant (e.g. conversion).  For selectable
// rounding modes instructions, funct3 gives the rounding mode
#define F5_ADD     0x0
#define F5_SUB     0x1
#define F5_MUL     0x2
#define F5_DIV     0x3
#define F5_SQRT    0xb
#define F5_SGNJ    0x4  // variants in funct3 - 0
#define F5_SGNJN   0x4  // - 1
#define F5_SGNJX   0x4  // - 2
#define F5_MIN     0x5  // variants in funct3 - 0
#define F5_MAX     0x5  // - 1

// convert instructions similarly have F2 tell you the size
// and RS2 tells you the variants, and funct3 gives you teh rounding mode
#define F5_CVTWS   0x18 //  variants in rs3, 0=W.S, 1=WU.S, 2=L.S, 3=LU.S, for sizes D and Q
#define F5_CVTSW   0x1a //  variants in rs3, 0=S.W, 1=S.WU, 2=S.L, 3=S.LU, for sizes D and Q
#define F5_CVTSD   0x80 //  dest size in rs3, 0=S, 1=D, 3=Q, F2 is source size
// this is really three kinds of cnversions I/U to/from F, and F2F and probably easier to handle
// than the above... 

// move + classify instructions, F2 gives size, can only be to/from 32 or 64 bit gpr, so no q version
#define F5_MOVXW    0x1c // move  (f->gpr), funct3=0
#define F5_CLASS    0x1c // move clasification to gpr (funct3=1)
#define F5_MOVWX    0x1e // move  (gpr->f). funct3=0

// compares, f2 gives size, funct3 gives kind of comparison
#define F5_CMP      0x14 // compare (funct3 = {0=>le, 1=>lt, 2=>eq})

static inline int decode_f2_to_size(uint8_t f2)
{
  return f2==F2_H ? 2 : f2==F2_W ? 4 : f2==F2_D ? 8 : f2==F2_Q ? 16 : -1;
}
...
static inline void decode_f3_to_round_mode(uint8_t f3)
{
    ...
    case 1: return FPVM_ROUND_ZERO; break;
    case 2: return FPVM_ROUND_NEGATIVE; break;
    case 3: return FPVM_ROUND_POSITIVE; break;
    case 4: return FPVM_ROUND_NEAREST_MAXMAG; break;
    case 7: return FPVM_ROUND_DYNAMIC; break;
    default: return FPVM_ROUND_DEFAULT; break;
    }
}
...
static inline void decode_4r(fpvm_inst_t *fi, riscv_inst_t *r)
{
  fi->operand_count=4;
  fi->operand_addrs[0] = r->r4.rd; // stash register number to ease later binding
  fi->operand_addrs[1] = r->r4.rs1;
  fi->operand_addrs[2] = r->r4.rs2;
  fi->operand_addrs[3] = r->r4.rs3;
}
...
{
  uint8_t f2, f3, f5, f7;
  uint8_t insize, outsize;
  
  riscv_inst_t ri;
  ri.val = ((riscv_inst_t*)(&fi->internal))->val;

  fi->common.is_vector = 0; // no vectors handled yet
  fi->common.has_mask = 0;  // no masks handled yet
  switch (r.a.opcode) {
  case OP_FOPGEN:
    f2 = F7_TO_F2(ri.r.funct7);
    f3 = ri.r.funct3;
    f5 = F7_TO_F5(ri.r.funct7);
    fi->common.op_size = decode_f2_to_size(f2);
    fi->common.dest_size = decode_f2_to_size(f2); // override later if needed
    switch (f5) {
    case F5_ADD:
      fi->common.op_type=FPVM_OP_ADD;
      fi->common.round_mode = decode_f3_to_round_mode(f3);
      decode_3r(fi,&ri);
      break;
    case F5_SUB:
      fi->common.op_type=FPVM_OP_SUB;
      fi->common.round_mode = decode_f3_to_round_mode(f3);
      decode_3r(fi,&ri);
      break;
    case F5_MUL:
      fi->common.op_type=FPVM_OP_MUL;
      fi->common.round_mode = decode_f3_to_roundmode(f3);
      decode_3r(fi,&ri);
      break;
    case F5_DIV:
      fi->common.op_type=FPVM_OP_DIV;
      fi->common.round_mode = decode_f3_to_roundmode(f3);
      decode_3r(fi,&ri);
      break;
...
the sign bit from rs1. For FSGNJ, the result’s sign bit is rs2’s sign
...
ry moves ry to rx (assembler pseudoinstruction FMV.S rx, ry); FSGNJN.S
rx, ry, ry moves the negation of ry to rx (assembler pseudoinstruction
FNEG.S rx, ry); and FSGNJX.S rx, ry, ry moves the absolute value of ry
to rx (assembler pseudoinstruction FABS.S rx, ry).
      */
      
    case F5_SGNJ:
    case F5_SGNJN:
    case F5_SGNJX:
      ERROR("SGNs are currently unhandled\n");
      return -1;
...
fpvm_inst_t *fpvm_decoder_decode_inst(void *addr)
{
  ...
  riscv_inst_t *r = (riscv_inst_t)addr;
  ...
  fi->internal = r->val; // stash a copy of the whole instruction
