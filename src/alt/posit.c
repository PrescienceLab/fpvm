...
#define POSIT_ENCODE(ITYPE, TYPE,  dest, result) {\
  ...
  *per_result = result;  \
  ...
  volatile ITYPE nan_encoded = NANBOX_ENCODE( (uint64_t) per_result, *(uint64_t*) &result_d);              \
  *(TYPE *)dest = result_d; \
...
#define POSIT_REVERT_SIGN(ITYPE, TYPE, ptr_val, dest, val_d) {\
  ...
  *_per_result = PNEGATE( *(PTYPE*) ptr_val );  \
  volatile ITYPE _nan_encoded = NANBOX_ENCODE( (uint64_t) _per_result, *(uint64_t*) &val_d);               \
  DEBUG("Nanbox result addr %p value %lf  \n", _per_result, PTD(*_per_result)); \
...
#define POSIT_BIN_OP(TYPE, ITYPE,NAME,OP,SPEC,ISPEC)				\
 int NAME##_##TYPE(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4) \
{									\
  ...
  if( ISNAN( *(uint64_t*) src1))  {src1 = (void* ) NANBOX_DECODE( *(uint64_t*) src1); op1 = *(PTYPE*)src1;}\
  else {op1 = PFD( *(TYPE*)src1 );}								\
  if( ISNAN( *(uint64_t*) src2))  {src2 = (void* ) NANBOX_DECODE( *(uint64_t*) src2);	op2 = *(PTYPE*)src2;}	\
  else {op2 = PFD( *(TYPE*)src2 );}								\
  								\
  double a = PTD(op1), b=PTD(op2); \
  DEBUG("In double %lf, %lf \n", a, b);\
  if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*) &a )) POSIT_REVERT_SIGN(ITYPE, TYPE, &op1, src_or1, a);\
  if( CORRUPTED(*(uint64_t*) src_or2,  *(uint64_t*) &b )) POSIT_REVERT_SIGN(ITYPE, TYPE, &op2, src_or2, b);\
  if( isnan(a ) ){ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src1, src_or1,  *(uint64_t*)src_or1); EXIT(1);}\
  if( isnan(b ) ){ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src2, src_or2,  *(uint64_t*)src_or2); EXIT(1);}\
...
{									\
  ...
  DEBUG( "src1 %p , %016lx,  src2 %p, %016lx \n, src3 %p, %016lx \n, ", src1, *(uint64_t*)src1, src2, *(uint64_t*)src2, src3, *(uint64_t*)src3);\
  if( ISNAN( *(uint64_t*) src1))  {src1 = (void* ) NANBOX_DECODE( *(uint64_t*) src1); op1 = *(PTYPE*)src1;}\
  else {op1 = PFD( *(TYPE*)src1 );}								\
  if( ISNAN( *(uint64_t*) src2))  {src2 = (void* ) NANBOX_DECODE( *(uint64_t*) src2);	op2 = *(PTYPE*)src2;}	\
  else {op2 = PFD( *(TYPE*)src2 );}								\
  if( ISNAN( *(uint64_t*) src3))  {src3 = (void* ) NANBOX_DECODE( *(uint64_t*) src3);	op3 = *(PTYPE*)src3;}	\
  else {op3 = PFD( *(TYPE*)src3 );}								\
  								\
  double a = PTD(op1), b=PTD(op2), c=PTD(op3); \
  DEBUG("In double %lf, %lf, %lf \n", a, b, c);\
  if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*) &a )) POSIT_REVERT_SIGN(ITYPE, TYPE, &op1, src_or1, a);\
  if( CORRUPTED(*(uint64_t*) src_or2,  *(uint64_t*) &b )) POSIT_REVERT_SIGN(ITYPE, TYPE, &op2, src_or2, b);\
  if( CORRUPTED(*(uint64_t*) src_or3,  *(uint64_t*) &c )) POSIT_REVERT_SIGN(ITYPE, TYPE, &op3, src_or3, c);\
  if( isnan(a ) ){ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src1, src_or1,  *(uint64_t*)src_or1); EXIT(1);}\
  if( isnan(b ) ){ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src2, src_or2,  *(uint64_t*)src_or2); EXIT(1);}\
  if( isnan(c ) ){ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src3, src_or3,  *(uint64_t*)src_or3); EXIT(1);}\
  PTYPE result = posit128_##OP2##p128(\
              posit128_##NEGOP##p128(PFD(0.0), posit128_##OP1##p128(op1, op2)),\
              op3);\
  a = PTD(op1), b=PTD(op2), c=PTD(op3);\
  DEBUG(#NAME "_" #TYPE ": (" #NEGOP "( " SPEC " " #OP1 " " SPEC " ) ) " #OP2 " " SPEC " = "SPEC " [" ISPEC "] (%p)\n", a, b, c, PTD(result), *(ITYPE*)&result, dest ); \
  POSIT_ENCODE(ITYPE, TYPE, dest, result);\
									\
  return 0;								\
}
...
static inline PTYPE posit_max(PTYPE a, PTYPE b) {
  ...
  if (PTD(a) > PTD(b)) { return a; } else {return b;} 
 ...
 int cmp_double(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4)
{
  ...
  *rflags &= ~(RFLAGS_OF | RFLAGS_AF | RFLAGS_SF | RFLAGS_ZF | RFLAGS_PF | RFLAGS_CF);
  ...
  DEBUG("double %s compare %lf %lf => flags %lx (%s)\n", special->unordered ? "unordered" : "ordered", a,b,*rflags,
	which==-2 ? "unordered" : which==-1 ? "less" : which==0? "equal" : "greater");
  #undef ITYPE
  #undef TYPE
  return 0;
}
...
#define DOUBLE_CONVERT_F2I(FTYPE,ITYPE,FSPEC,ISPEC)  { \
    ...
    DEBUG( "src1 %p , %016lx\n", src1, *(uint64_t*)src1);\
    if( ISNAN( *(uint64_t*) src1))  {src1 = (void* ) NANBOX_DECODE( *(uint64_t*) src1); op1 = *(PTYPE*)src1;} \
    else {op1 = PFD( *(FTYPE*)src1 );}								\
    double a = PTD(op1); \
    DEBUG("In double %lf\n", a);\
    if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*) &a )) POSIT_REVERT_SIGN(uint64_t, double, &op1, src_or1, a);\
    a = PTD(op1); \
 ...
 int f2i_double(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4) 
{
  ...
  case 1: DOUBLE_CONVERT_F2I(double,int8_t,"%lf","%hhd");
  case 2: DOUBLE_CONVERT_F2I(double,int16_t,"%lf","%hd");
  case 4: DOUBLE_CONVERT_F2I(double,int32_t,"%lf","%d");
  case 8: DOUBLE_CONVERT_F2I(double,int64_t,"%lf","%ld");
  default:
...
{
  switch (special->byte_width) {
  case 1: DOUBLE_CONVERT_F2I(double,uint8_t,"%lf","%hhu");
  case 2: DOUBLE_CONVERT_F2I(double,uint16_t,"%lf","%hu");
  case 4: DOUBLE_CONVERT_F2I(double,uint32_t,"%lf","%u");
  case 8: DOUBLE_CONVERT_F2I(double,uint64_t,"%lf","%lu");
  default:
...
{
  switch (special->byte_width) {
  case 1: CONVERT_F2I(float,int8_t,"%lf","%hhd");
  case 2: CONVERT_F2I(float,int16_t,"%lf","%hd");
  case 4: CONVERT_F2I(float,int32_t,"%lf","%d");
  case 8: CONVERT_F2I(float,int64_t,"%lf","%ld");
  default:
...
{
  switch (special->byte_width) {
  case 1: CONVERT_F2I(float,uint8_t,"%lf","%hhu");
  case 2: CONVERT_F2I(float,uint16_t,"%lf","%hu");
  case 4: CONVERT_F2I(float,uint32_t,"%lf","%u");
  case 8: CONVERT_F2I(float,uint64_t,"%lf","%lu");
  default:
...
{
  switch (special->byte_width) {
  case 4: DOUBLE_CONVERT_F2F(double,float,"%lf","%f");
...
{
  switch (special->byte_width) {
  case 8: CONVERT_F2F(float,double,"%f","%lf");
  default:
...
FPVM_MATH_DECL(add, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(sub, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(nmsub, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(msub, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(mul, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(div, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(max, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(min, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(sqrt, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(madd, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(nmadd, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
// FPVM_MATH_DECL(f2i, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
// FPVM_MATH_DECL(f2u, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(i2f, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(u2f, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
// FPVM_MATH_DECL(f2f, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
FPVM_MATH_DECL(cmp, float) { fprintf(stderr, "posit should not be invoked with floats"); return 0;}
...
if( ISNAN( *(uint64_t*) &a)) { \
        ...
        op1 = *(PTYPE*)xmm;\
        double tmp = PTD(op1);\
        op1 = ( CORRUPTED(*(uint64_t*) &a, *(uint64_t*)&tmp) ? PNEGATE( op1 ): op1);\
        a = PTD(op1);\
    }\
}
