  ...
  {                                                                            \
    ...
    *_per_result = -*(TYPE *)ptr_val;                                          \
    volatile ITYPE _nan_encoded =                                              \
        NANBOX_ENCODE((uint64_t)_per_result, *(uint64_t *)_per_result);        \
  ...
  int NAME##_##TYPE(op_special_t *special, void *dest, void *src1, void *src2, \
    ...
    *per_result = result;                                                      \
    volatile ITYPE nan_encoded =                                               \
        NANBOX_ENCODE((uint64_t)per_result, *(uint64_t *)&result);             \
...
// THIS DOES NOT HANDLE THE SPECIAL CASES OR RAISE EXCEPTIONS
int i2f_double(op_special_t *special, void *dest, void *src1, void *src2,
               void *src3, void *src4) {
...
// THIS DOES NOT HANDLE THE SPECIAL CASES OR RAISE EXCEPTIONS
int u2f_double(op_special_t *special, void *dest, void *src1, void *src2,
               void *src3, void *src4) {
  switch (special->byte_width) {
  case 1:
...
int ltcmp_double(op_special_t *special, void *dest, void *src1, void *src2,
                 void *src3, void *src4) {
  void *src_or1 = src1, *src_or2 = src2;
...
#define RECOVER(a, xmm)                                                        \
  {                                                                            \
    if (ISNAN(*(uint64_t *)&a)) {                                              \
      ...
      a = (CORRUPTED(*(uint64_t *)&a, *(uint64_t *)xmm) ? -*(double *)xmm      \
                                                        : *(double *)xmm);     \
    }                                                                          \
  }
