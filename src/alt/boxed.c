...
{									\
  ...
  if( ISNAN( *(uint64_t*) src2))  {src2 = (void* ) NANBOX_DECODE( *(uint64_t*) src2);	}	\
  								\
  if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*)src1)) IEEE_REVERT_SIGN(ITYPE, TYPE, src1, src_or1);\
  if( CORRUPTED(*(uint64_t*) src_or2, *(uint64_t*)src2)) IEEE_REVERT_SIGN(ITYPE, TYPE, src2, src_or2);\
  if( isnan(*(double*) src1) ) {ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src1, src_or1,  *(uint64_t*)src_or1); EXIT(1);}\
  if( isnan(*(double*) src2) ) {ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src2, src_or2,  *(uint64_t*)src_or2); EXIT(1);}\
...
{									\
  ...
  if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*)src1)) IEEE_REVERT_SIGN(ITYPE, TYPE, src1, src_or1);\
  if( CORRUPTED(*(uint64_t*) src_or2, *(uint64_t*)src2)) IEEE_REVERT_SIGN(ITYPE, TYPE, src2, src_or2);\
  if( CORRUPTED(*(uint64_t*) src_or3, *(uint64_t*)src3)) IEEE_REVERT_SIGN(ITYPE, TYPE, src3, src_or3);\
...
/* static int sqrt_double(void *dest, void *src1, void *src2, void *src3, void *src4) */
/* { */
...
#define DOUBLE_CONVERT_F2I(FTYPE,ITYPE,FSPEC,ISPEC)  { \
    ...
    DEBUG( "src1 %p , %016lx\n", src1, *(uint64_t*)src1);\
    if( ISNAN( *(uint64_t*) src1))  src1 = (void* ) NANBOX_DECODE( *(uint64_t*) src1);	\
    if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*)src1)) IEEE_REVERT_SIGN(uint64_t, double, src1, src_or1);\
    ITYPE result = (ITYPE) (*(FTYPE*)src1); \
    DEBUG("f2i[" #FTYPE " to " #ITYPE "](" FSPEC ") = " ISPEC " (%p)\n", (*(FTYPE *)src1),result,dest); \
    *(ITYPE*)dest = result; \
    return 0; \
...
//#define CONVERT_F2IT(FTYPE,ITYPE,FSPEC,ISPEC)  { \
    void* src_or1 = src1;\
    DEBUG( "src1 %p , %016lx\n", src1, *(uint64_t*)src1);\
    if( ISNAN( *(uint64_t*) src1))  src1 = (void* ) NANBOX_DECODE( *(uint64_t*) src1);	\
    if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*)src1)) IEEE_REVERT_SIGN(uint64_t, double, src1, src_or1);\
    ITYPE result = (ITYPE) (*(FTYPE*)src1 + 0.5*_SIGN(*(FTYPE*)src1)); \
    DEBUG("f2i[" #FTYPE " to " #ITYPE "](" FSPEC ") = " ISPEC " (%p)\n", (*(FTYPE *)src1),result,dest); \
    *(ITYPE*)dest = result; \
    return 0; \
}
 ...
 int cmp_double(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4)
{
  ...
  if( CORRUPTED(*(uint64_t*) src_or1, *(uint64_t*)src1)) IEEE_REVERT_SIGN(uint64_t, double, src1, src_or1);
  if( CORRUPTED(*(uint64_t*) src_or2, *(uint64_t*)src2)) IEEE_REVERT_SIGN(uint64_t, double, src2, src_or2);

  if( isnan(*(double*) src1) ) {ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src1, src_or1,  *(uint64_t*)src1); EXIT(1);}
  if( isnan(*(double*) src2) ) {ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src2, src_or2,  *(uint64_t*)src2); EXIT(1);}
 ...
 int ltcmp_double(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4)
{
  void* src_or1 = src1, *src_or2=src2;
  ...
  if( isnan(*(double*) src1) ) {ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src1, src_or1,  *(uint64_t*)src_or1); EXIT(1);}
  if( isnan(*(double*) src2) ) {ERROR("not my nan, try fix %p ori ptr %p, val %016lx\n", src2, src_or2,  *(uint64_t*)src_or2); EXIT(1);}
  DEBUG( "fault skip ? src1 %p , %lf,  src2 %p, %lf \n", src1, *(double*)src1, src2, *(double*)src2);
  double a = *(double*)src1;
  double b = *(double*)src2;
  int which;
	(void)which;
  ...
  DEBUG("done here\n");
  // DEBUG("double %s compare %lf %lf => flags %lx (%s)\n", special->unordered ? "unordered" : "ordered", a,b,*rflags,
	// which==-2 ? "unordered" : which==-1 ? "less" :  "equal/greater");
...
//si_int is int __powidf2 is using si_int;
