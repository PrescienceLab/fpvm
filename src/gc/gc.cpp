...
namespace gc {
  ...
  struct region {
    ...
    template<typename T>
    bool contains(T addr) const { return start >= (off_t)addr && (off_t)addr < end; }
...
static std::vector<gc::region> gc_find_regions() {
  // TODO: maybe cache this, and use calls to mmap, sbrk, munmap, mremap to invalidate it.
...
extern "C" unsigned fpvm_gc_run(void) {
  ...
  if (gcLogFile == NULL) {
    ...
    fprintf(gcLogFile, "time, instructions, freed, alive, gc_latency (us)\n");
  }

  // printf("[GC] swept %zu regions, %zu bytes (freed %d, %zu alive) in %lu us\n", regions.size(), total_size, freed, get_heap().size(), time_us() - start);
  fprintf(gcLogFile, "%lu, %lu, %u, %lu, %lu\n", now_ms - start_time, instructions, freed, get_heap().size(), time_us() - start);
  fflush(gcLogFile);
  instructions = 0;
  return freed;
}
