...
#define PRINT_PERFS(c) PRINT_PERF(c,gc); PRINT_PERF(c,decode_cache); PRINT_PERF(c,decode); PRINT_PERF(c,bind); PRINT_PERF(c,emulate); PRINT_PERF(c,patch);
#else
...
{
#define SHIMIFY(x) if (!(orig_##x = dlsym(RTLD_NEXT, #x))) { ERROR("Failed to setup SHIM for " #x "\n");  return -1; }
