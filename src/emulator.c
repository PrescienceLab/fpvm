...
static int bad(op_special_t *special, void *dest, void *src1, void *src2,
               void *src3, void *src4) {
...
typedef int (*op_t)(op_special_t *special, void *dest, void *src1, void *src2,
                    void *src3, void *src4);
...
int fpvm_emulator_emulate_inst(fpvm_inst_t *fi) {
  ...
  if (fi->common->is_vector) {
    ...
    src_step =
            ...
            ->op_size; // PAD: these can technically be different - FIX FIX FIX
  ...
  case FPVM_OP_F2UT:
    ...
    special.truncate = fi->common->op_type == FPVM_OP_F2IT ||
                       fi->common->op_type == FPVM_OP_F2UT;
  ...
  case FPVM_OP_U2FT:
    ...
    special.truncate = fi->common->op_type == FPVM_OP_I2FT ||
                       fi->common->op_type == FPVM_OP_U2FT;
    ...
    if (fi->common->op_size == 4) {
      ...
      func =
          vanilla_op_map[fi->common->op_type][1]; // PAD: why is this vanilla?
    } else {
      ERROR("Cannot handle f2u or f2i instruction with op_size = %d\n",
            fi->common->op_size);
      ASSERT(0);
  ...
  for (i = 0; i < count; i++, dest += dest_step, src1 += src_step,
      src2 += src_step, src3 += src_step, src4 += src_step) {
    DEBUG("calling %p((byte_width=%d,truncate=%d,unordered=%d), "
          "%p,%p,%p,%p,%p)\n",
          func, special.byte_width, special.truncate, special.unordered, dest,
          src1, src2, src3, src4);
...
#define _XMM(id) X86_REG_XMM##id
    int allxmm[32] = {
        _XMM(0),  _XMM(1),  _XMM(2),  _XMM(3),  _XMM(4),  _XMM(5),  _XMM(6),
        _XMM(7),  _XMM(8),  _XMM(9),  _XMM(10), _XMM(11), _XMM(12), _XMM(13),
        _XMM(14), _XMM(15), _XMM(16), _XMM(17), _XMM(18), _XMM(19), _XMM(20),
        _XMM(21), _XMM(22), _XMM(23), _XMM(24), _XMM(25), _XMM(26), _XMM(27),
        _XMM(28), _XMM(29), _XMM(30), _XMM(31)};
    for (int i = 0; i < 16; i++) {
  ...
  if (fi->common->is_vector) {
    ...
    dest_step = fi->common->op_size;
    src_step =
        fi->common->op_size; // these can technically be different - FIX FIX FIX
  }
