...
int fpvm_emulator_emulate_inst(fpvm_inst_t *fi)
{
  ...
  case FPVM_OP_F2UT:
    ...
    // PAD FIX by jiacheng, op count for dest is 0, replace fi->common->dest_size with what we
    // decoded previously
  ...
  for (i=0;i<count;i++,dest+=dest_step,src1+=src_step,src2+=src_step,src3+=src_step,src4+=src_step) { 
    DEBUG("calling %p((byte_width=%d,truncate=%d,unordered=%d), %p,%p,%p,%p,%p)\n", func,
...
int fpvm_fp_restore(fpvm_inst_t *fi, fpvm_regs_t *fr)
{
    ...
    if (fi->common->op_type == FPVM_OP_CALL){
      ...
      int allxmm[32] = {_XMM(0),_XMM(1),_XMM(2),_XMM(3),_XMM(4),_XMM(5),_XMM(6),_XMM(7),_XMM(8),_XMM(9),_XMM(10),
      _XMM(11),_XMM(12),_XMM(13),_XMM(14),_XMM(15),_XMM(16),_XMM(17),_XMM(18),_XMM(19),_XMM(20),_XMM(21),_XMM(22),_XMM(23),
      _XMM(24),_XMM(25),_XMM(26),_XMM(27),_XMM(28),_XMM(29),_XMM(30),_XMM(31) };
      for(int i=0;i<16;i++){
    ...
    //     func = op_map[fi->common->op_type][1];
    //   } else {
    //     ERROR("Cannot handle FPVM_OP_SHIFT_RIGHT_BYTE instruction with op_size = %d mnemonic=%d\n", fi->common->op_size, fi->common->op_type );
    //     return -1;
    //   }
    //   break;
    ...
    //     func = op_map[fi->common->op_type][1];
    //   } else {
    //     ERROR("Cannot handle FPVM_OP_SHIFT_LEFT_BYTE instruction with op_size = %d mnemonic=%d\n", fi->common->op_size, fi->common->op_type );
    //     return -1;
    //   }
    //   break;
    ...
    for (int i=0;i<count;i++,dest= increment(dest, dest_step), src1 = increment(src1, src_step), src2 =increment(src2, src_step),src3 =increment(src3, src_step), src4 =increment(src4, src_step)) { 
        DEBUG("calling %p((byte_width=%d,truncate=%d,unordered=%d), %p,%p,%p,%p,%p)\n", func,
        special.byte_width, special.truncate, special.unordered,
        dest,src1,src2,src3,src4);
        rc |= func( &special, dest,src1,src2,src3,src4);
