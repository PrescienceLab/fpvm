...
class VFG(ForwardAnalysis, Analysis):   # pylint:disable=abstract-method
        ...
        if job not in self._top_task.jobs:
            ...
            if unwind_count is None:
                ...
                for i in range(unwind_count):
                    ...
                    if not task.done:
                        l.warning("Removing an unfinished task %s. Might be a bug. With Unwind count %d", task, unwind_count)
    ...
    def _get_successors(self, job):
        ...
        if addr not in self._avoid_runs:
            ...
            if addr not in self._loops:
                ...
                # all_successors = job.sim_successors.unsat_successors + job.sim_successors.flat_successors + job.sim_successors.unconstrained_successors
                # all_successors = job.sim_successors.flat_successors + job.sim_successors.unconstrained_successors
                # if addr == 0x4012e5:
                #     print("hello")
            else:
                all_successors =  job.sim_successors.flat_successors
                # all_successors =  job.sim_successors.flat_successors + job.sim_successors.unconstrained_successors
    ...
    def _handle_successor(self, job, successor, all_successors):
        ...
        # if successor_addr == 0 and self._is_call_jumpkind(jumpkind):
        #     l.error(f"unknown function call; probably due to SYMBOLIC JUMP; SKIP call  {successor_addr}")
        #     return None
    ...
    def _job_queue_empty(self):
        ...
        if self._pending_returns:
            ...
            if pending_ret_key is None:
                ...
                l.error('No pending return for the current function %#x. Unwind the stack.', func_addr)
