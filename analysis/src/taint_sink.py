...
def taint_sink(proj, infos, sources, extern_functions, taint_whole_escape_functions, upto=1):
    ...
    nodes, keys, nodes_page_keys, pages, simp_keys , sim_procedures, callstacks = infos['nodes'], infos['keys'], infos['nodes_page_keys'],infos['pages'],infos['simp_keys'],infos['sim_procedures'], infos['callstacks']
    ...
    taint_func, extern_func = taint_functions(proj, sortedlist, extern_functions, taint_whole_escape_functions)
    ...
    for node_idx, (askey, blockid) in enumerate(sortedlist):
        ...
        if len(node.final_states) >= 1:
            ...
            if insn.mnemonic == 'call' or 'j' in insn.mnemonic : #call or any sorts of jumps
