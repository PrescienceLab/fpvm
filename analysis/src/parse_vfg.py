...
def analyze(binary_path, restart_analysis):
    ...
    # print("extern", extern_ranges)
    # exit()

    print("shared library", proj.loader.shared_objects)
    # exit()
    # cfg = proj.analyses.CFGEmulated(normalize=True, keep_state=True, enable_symbolic_back_traversal=True, max_iterations=10)
    main = proj.loader.main_object.get_symbol("main")
    # start = proj.loader.main_object.get_symbol("_start")
    # cfg = proj.analyses.CFGEmulated(normalize=True, keep_state=True, starts=[main.rebased_addr])
    # cfg = proj.analyses.CFGEmulated(normalize=True, keep_state=False)
    cfg = proj.analyses.CFGFast(normalize=True)
    print("cfg done ")

    # for key, func in proj.kb.functions.items():
    #     print(func)
    #     if func.addr == 7753584:
    #         call = proj.analyses.CallingConvention(cfg=cfg, func=func, analyze_callsites=True)
    #         print("yoyo")

    # cfg = proj.analyses.CFGEmulated(normalize=True, keep_state=True)
    start_state = proj.factory.blank_state(addr=main.rebased_addr)

    #skip this since for now, if and for is treated the same 
    # loop_finder = proj.analyses.LoopFinder(kb=cfg.kb, normalize=True, fail_fast=cfg._fail_fast)
    # loops = loop_finder.loops
    loops = []
 
    taint_whole_escape_functions_names = [ \
             'pow', 'log', 'log10', 'sqrt', '__powidf2', 'ldexp',\
             'exp', 'sin', 'cos', 'sincos', 'tan', 'asin', 'acos', 'atan', 'sinh', 'cosh', 'tanh', 'asinh', 'acosh', 'atanh', 'atan2', \
    ...
    # this is for enzo
    I_dont_want_to_analysis_set_func_name = ['fopen', 'fwrite', 'fread', 'fclose','fgetc', 'fputc', \
    'sscanf', \
    'unlink', \
    ...
    # try:
    #     os.remove(f"{name}.nodes.dat")
    #     os.remove(f"{name}.nodes.dir")
    #     os.remove(f"{name}.nodes.bak")
    #     os.remove(f"{name}.keys.bak")
    #     os.remove(f"{name}.keys.dat")
    #     os.remove(f"{name}.keys.dir")
    # except :
    #     pass
    ...
    if restart_analysis == 1:
        ...
        nodes = shelve.open(f"{name}-nodes", flag='r', writeback=True, debug=False, loadback=False, preset_keys=keys.keys)
        ...
        if os.path.isfile(f"{name}-extern-keys"):
            ...
            extern_keys = shelve.open(f"{name}-extern-keys", flag='r', writeback=True, loadback=False)
        ...
        pages = shelve.open(f"{name}-pages", flag='r', writeback=True, debug=False, loadback=True, buffer=30000, preset_keys=all_page_key)
        extern_keys.close()

        # pages = shelve.open(f"{name}-pages", flag='r', writeback=True, debug=False, loadback=True, buffer=50000)
...
if __name__ == '__main__':
    ...
    proj, infos, extern, taint_whole_escape_functions, known_functions_dict = analyze(sys.argv[1], int(sys.argv[2]))
    # exit()
    # nodes.sync()
    # keys.sync()
    # nodes.close()
    ...
    # sys.stdout = DevNull()
    sinks, func_sinks = taint_sink(proj, infos, sources, extern, taint_whole_escape_functions)
    # sys.stdout = sys.__stdout__
    print("=============== output sinks ==========")
