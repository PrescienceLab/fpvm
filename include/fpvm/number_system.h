 ...
 * 
 * To use a different number system, include your header above and change the prefix for function calls as per fp_ops.h
 * Assumes your functions are named $prefix_op_float and $prefix_op_double where op is add, sub, etc.
 * Functions must handle NaN boxing themselves.
 * Also should have an entry for calling your GC/allocator functions, if they exist
 * Example: ap_add_double -> FPVM_NUMBER_SYSTEM_INIT(ap)
 *
...
#define FPVM_MATH_DECL_VANILLA_ALT(op, type) \
    int vanilla_##op##_##type(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4);\
    int op##_##type(op_special_t *special, void *dest, void *src1, void *src2, void *src3, void *src4);
